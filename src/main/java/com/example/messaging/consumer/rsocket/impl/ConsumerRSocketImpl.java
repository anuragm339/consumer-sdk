...
public class ConsumerRSocketImpl implements RSocket {
    ...
    public ConsumerRSocketImpl(MessageHandler messageHandler) {
    ...
    private BatchMessage deserializeBatchMessage(Payload payload) {
        try {
            ...
            if (root.get("batchMessage") != null) {
                ...
                logger.debug("Received batch message with id: {} and count: {}", batchId, count);
    ...
    @Override
    public void dispose() {
        logger.info("Disposing RSocket connection");
    }
}
